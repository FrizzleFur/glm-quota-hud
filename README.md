...
```
V3 ⚡谷5h ██░░░░░░░░19% ↻18:37  📅周67% ↻1d→08/27  📈 58.8%/h 余81%≈1.4h ⚠️15:12  🗓 298.9%/d 余33%≈0.1d ⚠️08/26
V1 ⚡峰5h ███████░░░70% ↻19:28  🛠️ mcp8%  📈 89.3%/h 余30%≈0.3h ⚠️15:41
```

- `⚡谷5h` / `⚡峰5h` — 峰谷时段内联（工作日 14-18 点为峰，1x 积分；其余谷 0.5x）
- `🛠️ mcp8%` — MCP 周池调用余量（读数随用量分档变色）。V1 套餐专属池：V3（积分制）会话 API 不返回该池，但 MCP 调用（search-prime/zread 等）仍消耗 V1 周池，因此 V3 会话下自动从 V1 数据补渲染该段（2026-09-02 起，双账号都配置时始终可见）
- `↻18:37` / `↻1d→08/27` — 5h 池重置时刻；周池重置天数 + 绝对日期
- `📈 58.8%/h 余81%≈1.4h` — 近 45 分钟消耗速率 / 剩余量 / 按此外推还能用多久
- `🗓 298.9%/d` — 周池日均耗（跨天均值，随历史积累趋稳）
- `⚠️15:12` — 预计耗尽时刻早于重置（撑不到）；撑得到则显示 `✅`

任一额度池达 90%（`GLM_HUD_WARN` 可配）时进入预警形态，图标换 🔥 并弹一次 macOS 通知（携带完整预报，跨阈值只弹一次，回落自动复位）:

```
V1 🔥谷5h █████91%!0.4h ↻15:47  🛠️ mcp8%  📈 24.4%/h 余9%≈0.4h ⚠️14:11
```

通用版 `glm_quota_hud.py`（providers.json 配置 + CLI）格式:
...
以下能力目前仅在 `glm-usage.py`（作者自用深度定制版）：

- **速率预测** — 📈 5h 池近 45min 子窗口消耗速率（%/h）、🗓 周池日均耗（%/d）、剩余量外推还能用多久，✅ 撑得到重置 / ⚠️ 预计耗尽时刻
- **90% 阈值预警**（`GLM_HUD_WARN` 可配）— 状态栏 🔥 图标 + macOS 系统通知（latch 去重、回落自动复位、fire-and-forget 不占 HUD 3s 预算）
- **V3 会话跨账号补渲染 MCP 段**（2026-09-02）— 当前会话为 V3 时从 V1 数据补 `🛠️ mcp%`，`GLM_HUD_SHOW=all` / current=V1 时不重复
- 依赖 `patch-sgr.sh` v3 补丁（label 预算 50→150 UTF-16 单元）+ v4 补丁（sanitize 放行换行符，支持 extra label 两行显示）

### 两个脚本的关系

| 文件 | 定位 |
|------|------|
| `glm_quota_hud.py` | 通用发行版：providers.json 多供应商配置 + CLI 模式，适合公开分享 |
| `glm-usage.py` | 作者部署版：GLM 双账号硬编码直读环境变量，演进最快（JWT 回退链、MCP 周池、当前会话账号判定、速率预测、阈值预警） |

部署版的新能力稳定后会逐步回吸收进发行版。
...
**OSC / C0 / bidi / 其他转义照剥**——防注入能力不放松。

v4 补丁（2026-09-02）在此基础上放行换行符 `\n`（U+000A）：extra label 可分两行显示
（如 GLM 段 + 内存监控段各占一行），单行超终端宽度导致尾部段被 "..." 截断的问题随之消除。
`\r`（U+000D）仍剥（防 CRLF 双行注入），其余控制字符照剥。补丁幂等可重打：
